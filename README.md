...
Локальный русскоязычный голосовой помощник под Android. Работает прямо на телефоне: распознаёт команды, выполняет действия и отвечает голосом. Онлайн нужен только для сборки. Разрешение INTERNET используется исключительно для локальных сокетов встроенного Needle-сервера (127.0.0.1) — внешнего трафика приложение не создаёт.
...
- Системные действия: настройки (Wi-Fi, Bluetooth, звук, экран), веб-поиск, калькулятор, заметки и списки покупок, таймер и будильник, громкость (включая проценты)
- Календарь устройства: «что у меня сегодня», «что завтра», «что дальше»
- Музыка: «включи музыку» (NeonWave — плеер по умолчанию, дальше Spotify / Яндекс Музыка / системный плеер + play), пауза/стоп, следующий/предыдущий трек
- Хотворд «Юна»: «слушай юну» — foreground-сервис слушает даже со свёрнутым приложением, «хватит слушать» — стоп. Честно: ест батарею; Doze и Xiaomi могут прибивать сервис (дай приложению «без ограничений»); с выключенным экраном — best effort
...
- Уведомления: «прочитай уведомления», «следи за Telegram» (избранное озвучивается само), новые контакты анонсируются, «ответь маме иду» — ответ через кнопку уведомления
