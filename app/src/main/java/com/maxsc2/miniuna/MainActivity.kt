...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    private val pages by lazy {
        listOf(findViewById<View>(R.id.homePage), findViewById<View>(R.id.toolsPage),
            findViewById<View>(R.id.notesPage), findViewById<View>(R.id.settingsPage))
    }
    ...
    private fun setupNavigation() {
        ...
        findViewById<Button>(R.id.testTool).setOnClickListener {
            val result = intentEngine.classify("который сейчас час")
            findViewById<TextView>(R.id.toolStatus).text =
                "✓ Router test\n\"который сейчас час\" → " + result.intent + " (" +
    ...
    private fun showPage(index: Int) {
        ...
        navs.forEachIndexed { i, id -> findViewById<Button>(id).alpha = if (i == index) 1f else .55f }
    }
    ...
    private fun setupSettings() {
        ...
        tracking.setOnCheckedChangeListener { _, checked -> prefs.edit().putBoolean("tracking", checked).apply(); mascot.setTracking(checked) }
        breathing.setOnCheckedChangeListener { _, checked -> prefs.edit().putBoolean("breathing", checked).apply(); mascot.setBreathing(checked) }
        voice.setOnCheckedChangeListener { _, checked -> prefs.edit().putBoolean("voice", checked).apply() }
        ...
        emotions.forEach { pair ->
            emotionBox.addView(Button(this).apply {
                text = pair.second
                isAllCaps = false
                alpha = if (pair.first == prefs.getString("emotion", "neutral")) 1f else .7f
                setOnClickListener {
                    prefs.edit().putString("emotion", pair.first).apply()
                    mascot.setEmotion(pair.first)
                    for (i in 0 until emotionBox.childCount) emotionBox.getChildAt(i).alpha = .55f
                    alpha = 1f
                    mascot.blink()
                }
            })
    ...
    private fun applySavedMascotState() {
        ...
        applyPalette(paletteByName(prefs.getString("palette", "Синяя сфера")) ?: paletteByName("Синяя сфера")!!)
    }

    private fun applyPalette(p: Palette) {
        mascot.setPalette(Color.parseColor(p.core), Color.parseColor(p.mid), Color.parseColor(p.rim), Color.parseColor(p.aura))
    }
    ...
    private data class Palette(val name: String, val core: String, val mid: String, val rim: String, val aura: String)
    ...
    private fun handle(raw: String) {
        ...
        when (result.intent) {
            ...
            "SAVE_NOTE" -> {
                prefs.edit().putString("last_note", result.arguments["note"].orEmpty()).apply()
                refreshNote()
    ...
    private fun refreshNote() {
        ...
        findViewById<TextView>(R.id.noteText).text = if (note == null) "Пока пусто." else "Последняя заметка:\n\n" + note
    }
    ...
    override fun onInit(statusCode: Int) {
        if (statusCode == TextToSpeech.SUCCESS) tts.language = Locale("ru", "RU")
    }
